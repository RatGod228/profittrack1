...
const fetchExchangeRates = async (baseCurrency = 'EUR') => {
  const now = Date.now();
  // Cache for 1 hour
  if (exchangeRatesCache && exchangeRatesTimestamp > now - 3600000) {
    return exchangeRatesCache;
  }
  
  try {
    // Using exchangerate-api.com (free tier available)
    const apiUrl = EXCHANGE_API_KEY 
    ...
    if (response.rates) {
      exchangeRatesCache = response.rates;
...
const routes = {
  ...
  'GET /api/exchange-rate': async (req, res) => {
    try {
      ...
      if (!rate) {
        res.writeHead(400);
        return res.end(JSON.stringify({ error: 'Exchange rate not found' }));
      }
  ...
  'PUT /api/user/currency': async (req, res) => {
    ...
    try {
      ...
      if (!rate) {
        res.writeHead(400);
        return res.end(JSON.stringify({ error: 'Exchange rate not available' }));
      }
